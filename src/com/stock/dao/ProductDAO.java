...
import com.stock.ui.CurrentStocks;
...
//    addPurchaseDAO
        ...
        try {
            ...
            if (productDTO.getQuantity() > quantity) {
                ...
                try {
                    ...
                    String qry = "INSERT INTO issues(issuecode,date,productcode,customercode,quantity,revenue,soldby) VALUES('"+productDTO.getIssueCode()+"','" + sellDate + "','" + productCode + "','" + customersCode + "','" + qty + "','" + totalRevenue + "','" + username + "')";
                    stmt.executeUpdate(q);
    ...
    public ResultSet getSalesReportQueryResult() {
        try {
            String query = "SELECT salesid,issuecode,issues.productcode,productname,issues.quantity,revenue,soldby FROM issues INNER JOIN products ON issues.productcode=products.productcode";
            rs = stmt.executeQuery(query);
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public ResultSet getSearchSalesQueryResult(String searchTxt) {
        try {
            String query = "SELECT salesid,issuecode,issues.productcode,productname,quantity,revenue,soldby FROM issues INNER JOIN products ON products.productcode=issues.productcode INNER JOIN customers ON customers.customercode=issues.customercode WHERE issues.productcode LIKE '%" + searchTxt + "%' OR productname LIKE '%" + searchTxt + "%' OR soldby LIKE '%" + searchTxt + "%' OR fullname LIKE '%" + searchTxt + "%' OR issuecode LIKE '%" + searchTxt + "%' ORDER BY salesid";
            rs = stmt.executeQuery(query);
        } catch (SQLException e) {
            e.printStackTrace();
        }
